...
void CCamCal::process(void)
{
	...
	if (m_oCfg.getCalSel2dPtFlg())
	{
		...
		std::ofstream file("selected_points.txt");
		if (!file.is_open()) {
			std::cerr << "Error: Unable to open file for writing." << std::endl;
		} else {
			for (const auto& point : vo2dPt) {
				file << point.x << " " << point.y << std::endl;
			}
			file.close();
			std::cout << "Selected points saved to file: selected_points.txt" << std::endl;
		}
	}
